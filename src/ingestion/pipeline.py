...
def ingest_file(file_path: Path, data_root: Path) -> list[LetterRecord]:
    ...
    if suffix in SUPPORTED_DOCX:
        ...
        for seg_index, segment in enumerate(report.segments):
            raw_text_lines = [l.text for l in decoded_lines[segment.start_line:segment.end_line + 1]]
            ...
            low_confidence = [
                field for field, ok in meta.field_confidence.items() if not ok
            ]
...
def ingest_directory(data_root: Path) -> list[LetterRecord]:
    ...
    for path in sorted(data_root.rglob("*")):
        if path.is_file() and path.suffix.lower() in (*SUPPORTED_DOCX, *SUPPORTED_TXT):
